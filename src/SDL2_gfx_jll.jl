...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import SDL2_gfx_jll on Julia versions older than 1.3!")
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("323748d4-b4be-59be-a22c-1904c60787cb"))
platforms = [Pkg.Artifacts.unpack_platform(e, "SDL2_gfx", artifacts_toml) for e in artifacts["SDL2_gfx"]]
...
if best_platform === nothing
    @debug("Unable to load SDL2_gfx; unsupported platform $(triplet(platform_key_abi()))")
else
    # Load the appropriate wrapper
    include(joinpath(@__DIR__, "wrappers", "$(best_platform).jl"))
end

end  # module SDL2_gfx_jll
